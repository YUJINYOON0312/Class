...
desc employees;
--select 컬럼명,컬럼명 from 테이블이름 where 조건문 order by 정렬할대상컬럼명;
-- 모든컬럼을 출력하기위해 * 사용, 원하는 컬럼만 명시 조회 가능
...
-- 컬럼명읠 별칭으로 표기 가능 : 컬럼명 as 별칭 (as 는 생략해도 되요)
...
from employees
order by 5 desc;
--order by 5 : 5-컬럼의 출력순서로 정렬가능

select * from employees
where last_name='King'; -- 컬럼명 = 검색어 정확하게 일치하는데이터 검색
--문자열 '' java ==
--컬럼명이나 테이블이름은 대소문자 구분없이 사용함
