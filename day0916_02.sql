...
--테이블을 만들때 거의 모든 테이블은 PK 생성을 기본으로 하는걸로...
...
create table test04(
    ...
    writer varchar2(255),
    ---컬럼명과 타입을 모두 정의한 이후 정의할수 있다.
    PRIMARY KEY(no)-- 제약조건이름은 시스템이 자동으로부여 : SYS_숫자
);
...
insert into emp VALUES(2201, '이름1', 10);--dept테이블의 pk컬럼에 존재하는 데이터만 넣어주세요
...
--추가 제약이 없는경우 null 허용합니다.

--fk연결되어있는 데이블은 조인(join)을 통해서 조회할수 있어요
...
--중복데이터 불가합니다.
--자동으로 index를 만들어줍니다.
--null 허용하고 여러개 넣을수 있어요: not null을 지정하면 null 허용하지 않아요
create table member(
...
create table member2(
    mno number primary key,
    email varchar2(40) UNIQUE,--system이 이름 만들어줘요
    name varchar2(40)
);
--4. not null 제약조건
...
create table member4(
    mno number primary key,
    email varchar2(40) UNIQUE not null,--system이 이름 만들어줘요
