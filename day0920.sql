...
-- employee_id, email, job_title 을 조회
--oracle
...
-- NATURAL join은 별칭을 사용할 필요가 없음
-------------------------------------------------------------------------
select employee_id, email, job_id, job_title
from employees
join jobs 
using (job_id); --양쪽테이블의 job_id로 조인
--using 절에 조인대상 (컬럼)을 지정
--단 컬럼이름은 동일한 이름으로 만들어져야함
--------------------------------------------------
--#####EQUI JOIN#######
-- join on
-- NATURAL JOIN
...
-- join using
-- FK가 null 인경우 조인조건은 조회되지 않음: 조인대상이 존재하지 않음
---------------------------------------------------
--###### OUTER JOIN #######
--RIGHT OUTER JOIN
--oracle
...
-- 단일행 비교연산자: = , >, >=,<,<= ,!=,<>
-- 서브쿼리의 실행결과가 하나의 행만 나와야 함
-- 서브쿼리에서 그룹함수를 결과로하는 쿼리
-- 유일한요소(PK) 와 동등조건비교 

-- employee_id가 120인 사원의 부서의 사원중 전체 평균급여보다 많이 받는 사원의
...
---다중컬럼 (조회결과가 2개이상인경우 )
--pairwise 다중컬럼 서브쿼리
--대상 컬럼개수랑 서브쿼리 실행결과의 컬럼 개수 같아야 함
select * from employees
